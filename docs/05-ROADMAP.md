...
- **Quality Gate** — was `NONE` at the time of PR #6's first verification scan; the default "Sonar way" gate is now in effect (it Quality-Gate-Failed PR #6 with 11.6% new-code duplication, which forced PR #6 to add `sonar.cpd.exclusions` for `internal/remediation/guidance.go`, `cmd/imports.go`, and `**/*_test.go` to bring the gate green). **Action remaining:** verify the assigned gate is the project owner's preference; revisit the cpd exclusions in any future refactor PR (especially the verifier-httpapi extraction listed under P2).
- **`sonar-project.properties`** — added in PR #6 as part of resolving the Quality Gate failure. Drives `sonar.cpd.exclusions`, `sonar.coverage.exclusions`, and `sonar.exclusions` for SonarCloud's Automatic Analysis. **Done.**
- **`.github/workflows/sonar.yml`** — still missing. Automatic Analysis currently picks up the repo without an explicit workflow, but a dedicated workflow with `SonarSource/sonarqube-scan-action` would enable per-PR coverage upload and tighter feedback loops. **Action:** add as a small chore PR once `SONAR_TOKEN` is configured as a repo secret.
